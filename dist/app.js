...
var mainState = {
  ...
  preload: function () {
    ...
    game.load.atlasJSONHash('zombie', 'assets/sprites/zombie.png', 'assets/sprites/zombie.json');
  ...
  initZombies : function (zombieCount) {
    ...
    for (var i = 0; i < zombieCount; i++) {
      ...
      this.zombies[i].scale.x = 0.5;
      this.zombies[i].scale.y = 0.5;
      this.zombies[i].animations.add('walk');
  ...
  checkAggro : function(min_distance) {
    for (var i = 0; i < this.zombies.length; i++) {
      ...
      if (distance <= min_distance) {
        zombie.rotation = game.physics.arcade.angleToXY(zombie, this.player.body.x, this.player.body.y) + game.math.degToRad(90);
